...
def plot_clustering(TFbox, mode = None, order = 'standard', showfig = True, ax = None):
    ...
    Parameters:
        ...
        mode: None, '1d', '2d' (default None)
            ...
            theeta coefficients will be plotted. If '2d' alpha and theta coefficients
        ...
        order: 'standard' or 'sorted' (default 'standard')
            Way to plot the coefficients (only when mode='1d' or the methos 
            containei in the TFbox is 1 or 2, otherwise is not considered)
    ...
    Returns:
        fig: instance of Figure
            Figure representig the clustering
        
    """
    
    if ((mode == '2d' or (mode is None and TFbox['method'] in [3,4])) and order=='sorted'):
        warnings.warn("order can only be 'standard' if mode is '2d' or mode is None and method in TFbox is 3 or 4. Automatically switched to 'standard'")
        order = 'standard'
    ...
    if ((mode == '1d') or ((mode is None) and (method in [1, 2]))):
        ...
        if method in [1,2,3,4]: ax.set_title('Method '+ str(method)+': '+meth_names[method-1])
        else: ax.set_title('Method '+ str(method)) 
        ax.legend()
...
def plot_coefficients(psds, freqs, ch_names, theta_range = None, alpha_range = None, mode = '2d', order = 'standard', showfig=True, ax = None):
...
def plot_chs(TFbox, ch_locs, mode = None, showfig = True, ax = None):
    ...
    if method in [1,2,3,4]: title = 'Method '+ str(method)+': '+meth_names[method-1]
    else: title = 'Method '+ str(method)
...
def plot_psds(psds, freqs, average = True, showfig = True, ax = None):
    ...
    if average is True:  ax.plot(freqs, psds.mean(axis=0))
    if average is False:  ax.plot(freqs, psds.T)
    ax.grid()
